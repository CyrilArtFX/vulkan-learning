...
void VulkanRenderer::draw()
{
	...
	vkAcquireNextImageKHR(mainDevice.logicalDevice, swapchain, std::numeric_limits<uint32_t>::max(),
		imageAvailable[currentFrame], VK_NULL_HANDLE, &imageToBeDrawnIndex);
...
void VulkanRenderer::createGraphicsPipeline()
{
	...
	*/
	...
	// -- PIPELINE LAYOUT --
	std::array<vk::DescriptorSetLayout, 2> descriptorSetLayouts{ descriptorSetLayout, samplerDescriptorSetLayout };
...
void VulkanRenderer::createRenderPass()
{
	...
	std::array<vk::AttachmentDescription, 3> renderPassAttachments{ colorAttachment, depthAttachment, colorAttachmentResolve };
...
void VulkanRenderer::createFramebuffers()
{
	...
	{
		// Setup attachments
		std::array<vk::ImageView, 3> attachments{ colorImageView, depthBufferImageView, swapchainImages[i].imageView };
...
void VulkanRenderer::createDescriptorSetLayout()
{
	...
	std::vector<vk::DescriptorSetLayoutBinding> layoutBindings{ vpLayoutBinding, modelLayoutBinding };
	*/

	std::vector<vk::DescriptorSetLayoutBinding> layoutBindings{ vpLayoutBinding };
	...
	std::vector<vk::DescriptorSetLayoutBinding> samplerLayoutBindings{ samplerLayoutBinding };
...
void VulkanRenderer::createDescriptorSets()
{
	...
	std::vector<vk::DescriptorSetLayout> setLayouts(swapchainImages.size(), descriptorSetLayout);
	...
	for (size_t i = 0; i < swapchainImages.size(); ++i)
	{
		...
		*/
		...
		std::vector<vk::WriteDescriptorSet> setWrites{ vpSetWrite };
...
void VulkanRenderer::updateUniformBuffers(uint32_t imageIndex)
{
	...
	memcpy(data, &viewProj, sizeof(ViewProjection));
...
{
	std::vector<vk::Format> formats{
		vk::Format::eD32SfloatS8Uint, // Look for a format with 32bits death buffer and stencil buffer
		vk::Format::eD32Sfloat, // if not found, without stencil
		vk::Format::eD24UnormS8Uint // if not 24bits depth and stencil buffer
	};

	vk::Format depthFormat = chooseSupportedFormat(formats, vk::ImageTiling::eOptimal,
		vk::FormatFeatureFlagBits::eDepthStencilAttachment); // Format supports depth and stencil attachment
...
VkImage VulkanRenderer::createImage(uint32_t width, uint32_t height, uint32_t mipLevels,
...
vk::Format VulkanRenderer::chooseSupportedFormat(const std::vector<vk::Format>& formats, vk::ImageTiling tiling,
...
stbi_uc* VulkanRenderer::loadTextureFile(const std::string& filename, int* width, int* height, vk::DeviceSize* imageSize)
{
...
int VulkanRenderer::createTextureImage(const std::string& filename, uint32_t& mipLevels)
{
	...
	texImage = createImage(width, height, mipLevels, vk::SampleCountFlagBits::e1, vk::Format::eR8G8B8A8Unorm,
